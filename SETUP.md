...
稼働状況ページへの登録・送信は、登録済みSlackアプリの認証から起動時に自動で設定されます。
監視のための `zerochan cloud login` や、各PCへのSupabase認証情報の入力は不要です。
既存PCは `zerochan update` 後、稼働中アプリから自動送信します。停止中アプリは対象フォルダで起動してください。
クラウド引き継ぎ機能の認証は別機能であり、この自動登録では変更しません。
