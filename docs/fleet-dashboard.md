...
所属は**起動フォルダ名、または現在対応中のフォルダ名**で自動判定する。Slackアプリの手動所属登録、GitHub origin、Gitの有無には依存しない。
照会元はSlackを受け付けたプロジェクトフォルダ名で固定し、モデルや投稿本文に別フォルダを選ばせない。
同じSlack workspace/space内で、NFC正規化後のフォルダ名を完全一致で照合する。同名の別パスも同じグループとして扱う。
起動BSB・現在OtherのアプリはBSBの一覧に含めるが、Otherの作業内容・件数・時刻は返さず開始待ちとして扱う。
現在Otherへの照会ではOtherの作業要約だけを返す。現在フォルダと要約は同じ実行中jobから取得し、実行中がなければ先頭の開始可能な待機jobを使う。

配備時に `202609260003_fleet_folder_membership.sql` を適用し、Workerと各PCを更新する。
通常起動後のheartbeatで起動・現在フォルダが自動報告される。利用者が登録SQLを生成・実行する手順はない。
旧版PCのフォルダ報告は更新まで一覧へ含めない。旧管理者所属表はrollback用に残るが照会では参照しない。
別フォルダへ切り替わった報告は現在のgeneration/sequenceと照合し、古い報告を現行の作業として使わない。
90秒以上古い報告、切断、重複アプリ、世代が切り替わった報告は「空き」としない。
クラウドやフォルダ報告の確認に失敗した場合は確認不能と答え、ローカル情報で補完しない。
