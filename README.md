...
第2回レビュー中のrepository変更は台帳破損や未回答とは区別します。保存済み回答は同じbindingの
poll／retryで再取得でき、全回答取得済みなら外部枠を再起動しません。broker再起動後も同じです。
`repositoryAssessmentRequired`が返る場合、メインCodexが途中差分の関連性と必要な対象テストを確認して
続行します。回答の配送完了は、変更後コードの自動承認ではなく、追加のreview roundも要求しません。
