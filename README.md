...
要求された3者の回答が揃うまでは、設計・レビューを完了扱いにしません。未回収時はSlackに
担当名と原因を知らせ、取得済み回答と作業を保持して待機します。認証切れと起動・通信失敗は区別します。
同じ実行内の復旧では、終了確認済みの未回収枠だけを再実行し、保存済みの回答は再利用します。
回答本文はowner-onlyのローカルstateへ保存します。復旧だけの追加入力かどうかはメインCodexが判定し、
