...
3枠はbest-effortで各1回試行し、回答が0件でも欠員だけを理由に本作業を停止しません。未回収時はSlackに
担当名と原因を知らせ、取得済み回答と主担当の確認を基に継続します。認証切れと作業フォルダ・起動・通信失敗は区別します。
終了済みroundの再pollや旧セッションのretry指定でも、欠員を埋めるための再起動は行いません。
