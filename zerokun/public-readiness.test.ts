...
describe('public Codex defaults', () => {
  ...
  test('repository未取得の新規Macにもmain branch bootstrapの取得手順がある', () => {
    ...
    expect(codexVersion).toContain('ZEROKUN_MIN_CODEX_VERSION="0.149.0"')
    ...
    expect(rootReadme).toContain('旧PCのgatewayを停止して新PCへ移行')
    expect(rootReadme).toContain('既存Slack Appの`xapp-...`と`xoxb-...`を再利用できます')
    expect(rootReadme).toContain('稼働するPCごとに別のSlack Appとtoken')
    expect(rootReadme).toContain('履歴下限')
