...
describe('Codex branch self update', () => {
  ...
  test.skipIf(process.platform !== 'darwin')('candidate sandboxのselected Git検証はstaged-only末尾空白も更新前に拒否する', () => {
    ...
    writeFileSync(join(verifyDir, 'verify.sh'), [
      ...
      '/usr/bin/env -i PATH=/usr/bin:/bin HOME=/var/empty TMPDIR=/var/empty LANG=C LC_ALL=C TERM=dumb GIT_CONFIG_NOSYSTEM=1 GIT_CONFIG_GLOBAL=/dev/null GIT_TERMINAL_PROMPT=0 GIT_ASKPASS=/usr/bin/false SSH_ASKPASS=/usr/bin/false GIT_PAGER=cat GIT_OPTIONAL_LOCKS=0 "$candidate_git" --no-pager -c core.fsmonitor=false diff --quiet --no-ext-diff -- version.txt </dev/null',
      'if /usr/bin/env -i PATH=/usr/bin:/bin HOME=/var/empty TMPDIR=/var/empty LANG=C LC_ALL=C TERM=dumb GIT_CONFIG_NOSYSTEM=1 GIT_CONFIG_GLOBAL=/dev/null GIT_TERMINAL_PROMPT=0 GIT_ASKPASS=/usr/bin/false SSH_ASKPASS=/usr/bin/false GIT_PAGER=cat GIT_OPTIONAL_LOCKS=0 "$candidate_git" --no-pager -c core.fsmonitor=false diff --cached --quiet --no-ext-diff HEAD -- version.txt </dev/null; then echo "error: staged-only fixture is not staged" >&2; exit 43; fi',
      'candidate_git_diff_check',
      '',
    ].join('\n'))
    must(['git', 'add', '.'], fixture.repo.seed)
