...
describe('Zero-kun watchdog', () => {
  test('selftestで連続down・再通知抑制・復旧・muteの状態遷移を検証する', () => {
    ...
    expect(stdout).toContain('ok: hourly reminder remains suppressed')
    expect(stdout).toContain('ok: daily reminder remains suppressed with legacy configuration')
    expect(stdout).toContain('ok: partial recovery does not repeat an already notified severity')
    expect(stdout).toContain('ok: full recovery re-arms the next outage notification')
    expect(stdout).toContain('ok: failed first delivery is retried')
    expect(stdout).toContain('ok: failed escalation delivery is retried')
    expect(stdout).toContain('ok: failed recovery delivery is retried')
